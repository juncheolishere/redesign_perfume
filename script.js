...
const renew = () => {
    ...
    {
        wrap1.style.backgroundColor = "white";
        mainPcImg.children[0].style.transform = "scale("+(0.5+k/2000)+","+(0.5+k/2000)+")";
        mainPcImg.style.opacity = 1;
    }
    else if (k>2000 && k <= 4000)
    {
        ...
        mainPcImg.style.opacity = 1 - (k-2000)/1500;
    }
    else if (k>4000)
    {
        wrap1.style.backgroundColor = "rgba(246,243,240,255)";
...
new TypeIt(perfumeComment2[0], {
    ...
    .pause(500)
    .type(" 저 멀리 누군가의 상쾌한 비누향기가 내 코 끝에 와닿는다.'")
    .go();
...
new TypeIt(perfumeComment2[1], {
    ...
    .type("'시원하게 한차례 소나기가 지나간 공원에서 느껴지는", { delay: 300 })
    .pause(500)
...
new TypeIt(perfumeComment2[8], {
    ...
    .pause(500)
    .type(" 그 푸른빛의 시린 공기가 바닥에 닿아 꽃 처럼 부서지는 얼음'")
    .pause(500)
    .go();
...
new TypeIt(perfumeComment2[10], {
    ...
    .type("'고개를 돌려보니 갈대숲을 얼추 닮은 숲과 언덕이 한 번도 본 적 없지만,", { delay: 300 })
    .pause(500)
    .type(" 어딘가 이국적인 편안함을 가져다 주는 자연 풍경'")
    .pause(500)
    .go();
...
new TypeIt(perfumeComment2[13], {
    ...
    .pause(500)
    .type(" 뺨을 스치는 기분 좋은 신선함에 저절로 지어지는 미소'")
    .pause(500)
    .go();
...
new TypeIt(perfumeComment2[14], {
    ...
    .type("'드럼세탁기에서 따듯한 바람으로 살균까지 마치고-", { delay: 300 })
    .pause(500)
    .type(" 막 나온 촉촉한 새하얀 호텔 수건'")
    .pause(500)
    .go();
